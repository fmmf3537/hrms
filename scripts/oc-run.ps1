# oc-run.ps1 — 以后台方式运行 opencode run 执行一个开发切片（与 dsh-run.ps1 同约定）
# 用法: powershell -NoProfile -File scripts/oc-run.ps1 -SliceId M5-2-D2 -PromptFile docs/cursor-prompts/M5-2-D2.md
# 产物: logs/oc/<SliceId>.log（实时输出）+ logs/oc/<SliceId>.exitcode + <SliceId>.pid
param(
