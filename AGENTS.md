...
- ⏳ **M5 联调上线（第 16-18 周，进行中）**：M5-1 Nginx+Docker ✅ → M5-2 前端（0 基础设施 ✅ / A1 ✅ / A2 ✅ / B 考勤 ✅ / **C1 薪酬基础配置 ✅**（薪级薪档+方案+社保公积金+个税工具，22 端点，56/56 前端测试）/ **C2 算薪与工资条 ✅**（算薪 Run 状态机+HR 工资单+ESS 工资条+三类导出，19 端点，62/62 前端测试）/ **C3 提成·预警·调薪 ✅**（提成汇总与结算+成本预警+调薪全流程，22 端点，70/70 前端测试，M4 薪酬前端收官）/ **D1 绩效基础配置 ✅**（周期+指标库+方案+系数+等级阈值，12 端点，74/74 前端测试，DSH headless 首跑）/ **D2 考核记录流程 ✅**（自评→经理→校准→HR→CEO→归档六环节状态机，16 端点，78/78 前端测试，opencode 首跑+2 轮修订）/ **D3 等级计算与绩效奖金 ✅**（单笔/批量等级计算+部门比例校准+兑现配置+奖金计算/预发/季度结算+奖金单，11 端点，82/82 前端测试，Codex 首跑+1 轮修订）/ **D4 销售提成 ✅**（产品字典与比例配置+回款登记与财务确认+提成计算，8 端点，86/86 前端测试，opencode+2 轮修订）/ **D5 申请联动 ✅**（调薪提议审批+晋升提名+PIP 触发与月度评审，8 端点，90/90 前端测试，opencode 首跑一次通过；调薪/晋升 audit-log 存储形态、ESS 本人过滤、后端 employee 无隔离缺口留 M5-04），**M5-2 前端收官**）→ M5-01~08 联调任务（**M5-01 接口联调 ✅**（Postman 生成器+185/185 覆盖+newman Auth 5/5 真实联调；修复 BullMQ maxRetriesPerRequest 启动崩溃；发现强制改密拦截为设计行为）/ **M5-02 Playwright E2E ✅**（登录/打卡/请假/算薪/绩效 5 主流程 6 用例全绿 19.9s，真实双端；token 注入+限流规避；降级记录 4 个既有缺陷根因：绩效列表信封错位、打卡漏 wifiSsid、审批流 key 不匹配、算薪前置档案缺失）/ **M5-03 数据迁移 ✅**（Excel 模板生成器+导入校验脚本+运行手册三件套，复用 createEmployee 全链路，40 行合成数据端到端自测：导入 40 成功/双跑幂等 40 跳过/purge 清零/加密回环验证；真实 40 人 Excel 待 HR 提供后人工导入） / **M5-05 压测 ✅**（Artillery 框架+5 场景+token 前置复用可用，429 根因（apiLimiter 1min/100 次与 100VU 冲突）移交 M5-09 缺陷 5 修复后重跑；commit 78cdcc5）/ 渗透 / 培训 / 并行核算 / 上线 待做）
