...
- ⏳ **M5 联调上线（第 16-18 周，进行中）**：M5-1 Nginx+Docker ✅ → M5-2 前端（0 基础设施 ✅ / A1 ✅ / A2 ✅ / B 考勤 ✅ / **C1 薪酬基础配置 ✅**（薪级薪档+方案+社保公积金+个税工具，22 端点，56/56 前端测试）/ **C2 算薪与工资条 ✅**（算薪 Run 状态机+HR 工资单+ESS 工资条+三类导出，19 端点，62/62 前端测试）/ **C3 提成·预警·调薪 ✅**（提成汇总与结算+成本预警+调薪全流程，22 端点，70/70 前端测试，M4 薪酬前端收官）/ **D1 绩效基础配置 ✅**（周期+指标库+方案+系数+等级阈值，12 端点，74/74 前端测试，DSH headless 首跑）/ **D2 考核记录流程 ✅**（自评→经理→校准→HR→CEO→归档六环节状态机，16 端点，78/78 前端测试，opencode 首跑+2 轮修订），D3 等级与奖金 / D4 销售提成 / D5 申请联动 待做）→ M5-01~08 联调任务（接口联调 / E2E / 数据迁移 / 渗透 / 压测 / 培训 / 并行核算 / 上线）
