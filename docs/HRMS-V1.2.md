...
| C8 | 薪酬数据加密 + 二次授权 **本切片由 M0.5-3 + M0.5-7 业务 commit 实现（薪酬数据加密 + 改密 + 2FA），M4-C8 业务 commit 实现调薪实际执行（V1.2 §二.4.5 + M3-wrap-up §7 遗留 D6 联动）** | 3 | 2d |
...
| **M4.0.8 / C8.0.1** | **2026-08-28** | **feat(salary): M4-C8 调薪实际执行（联动 D6 + 写 employee_salary_history）（Cursor 交付）<br>**范围**：salary_adjustments 1 张表 + 8 端点（`/api/salary/adjustments/*`）+ 5 权限点 + 10 错误码（73801-73810）+ 6 项 salary.adjustment.* configs + 4 种调薪类型 + 3 级审批（hr 兼任复核 → CEO）+ 执行写 employee_salary_history / 同步 employee_salary_plans / 晋升联动 position_history<br>**强约束**：不创建 BullMQ 队列（仅暴露 executePendingAdjustments）；不给 employees 加 baseSalary；不改 D6 3 service（仅 prisma 读 audit_logs）；不联动 C4 算薪；D1-D6 + C1-C7 共 32 service + M0.5 4 service = 36 service 0 行改动；5 角色无 finance；V1.2 切片表 C8 加密+2FA 已由 M0.5-3 + M0.5-7 实现 | **Cursor + WorkBuddy** |
